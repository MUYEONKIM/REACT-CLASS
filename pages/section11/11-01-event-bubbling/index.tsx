    ...
    <>
      <div>
        {data?.fetchBoards.map(
          (
            el: any // 함수타입은 인자 받는 곳에서 타입을 선언해주면됨
          ) => (
            <div>
              <span>
                <input type="checkbox" />
              ...
              <span id={el.number} onClick={onClickSpan} style={mystyle}>
                {el.writer}
              </span>
              <span
                id={el.number}
      ...
      <div>
        {data?.fetchBoards.map(
          (
            el: any // 위에서 처럼 하나하나 지정 말고 한번에 지정하는 법 (이벤트 버블링으로 클릭 이벤트 전파)
          ) => (
            <div id={el.number} onClick={onClickDiv}>
              <span>
                <input type="checkbox" />
              </span>
              <span onClick={qqq} style={mystyle}>
                {el.number}
              </span>
